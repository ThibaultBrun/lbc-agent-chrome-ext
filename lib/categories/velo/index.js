...
async function enrichAd({ ad, llm, settings, emit, log, phase }) {
  ...
  // 1bis) Extraction des specs DIRECTEMENT depuis l'annonce LBC (description vendeur).
  // Source la plus fiable : c'est ce velo precis, pas une autre version.
  let adSpecs = null;
  try {
    const out = await llm.json({
      system: "Tu extrais les caracteristiques techniques d'un velo a partir d'une annonce d'occasion. Tu ne te bases QUE sur le texte de l'annonce, jamais sur une connaissance generique du modele. Tu reponds uniquement en JSON strict.",
      prompt: `Extrais les caracteristiques techniques de ce velo precis depuis son annonce.

REGLE STRICTE : ne te base QUE sur le texte de l'annonce. Si une info n'est pas
explicitement mentionnee, mets null. N'invente RIEN, ne devine RIEN, ne complete
PAS avec ta connaissance generale du modele.
...
"""`,
    ...
    if (out) {
      adSpecs = out;
      const filled = countFilledSpecs(adSpecs);
      log(`[specs:from-ad] ${filled} caracteristiques extraites de l'annonce LBC`);
    }
  } catch (e) {
    log(`[specs:from-ad:error] ${e.message}`);
  }
  ...
  // Fusion des specs : annonce LBC (priorite ABSOLUE) > constructeur > magazine > retailer.
  // L'annonce decrit ce velo PRECIS (pas une autre version du modele), donc ses
  // specs sont la verite terrain qui doit ecraser tout le reste.
  const specsCandidates = [];
  if (adSpecs) {
    specsCandidates.push({ specs: adSpecs, profile: { type: "ad", name: "Annonce LBC" } });
  }
  for (const r of webResults) {
    if (r.specs_in_page) {
      specsCandidates.push({ specs: r.specs_in_page, profile: { type: r.source_type, name: r.source_name } });
    }
  }
  // Tri : annonce (type='ad' → priorite 0) en premier, puis constructeur=1, magazine=2, etc.
  specsCandidates.sort((a, b) => {
    const pa = a.profile.type === "ad" ? 0 : specsSourcePriority(a.profile.type);
    const pb = b.profile.type === "ad" ? 0 : specsSourcePriority(b.profile.type);
    return pa - pb;
  });
  const mergedSpecs = mergeSpecs(specsCandidates);
  if (mergedSpecs) {
    const filled = countFilledSpecs(mergedSpecs);
    const sources = specsCandidates.map((c) => c.profile.name).join(", ");
    log(`[specs:merged] ${filled} caracteristiques techniques agregees depuis : ${sources}`);
  } else {
  ...
  // Sanity check : si Nano a mis 'LBC' / 'Leboncoin' / 'Troc Velo' / 'Comparables'
  // comme retail_source, c'est qu'il a halluciné le prix retail depuis un comparable.
  // On nettoie : retail_eur et retail_source forces a null.
  if (synth.retail_source) {
    const lowSrc = synth.retail_source.toLowerCase();
    if (/lbc|leboncoin|troc|comparable/.test(lowSrc)) {
      log(`[retail:hallucine] LLM mis retail_source="${synth.retail_source}" depuis un comparable, on nettoie`);
