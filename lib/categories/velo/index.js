...
async function enrichAd({ ad, llm, settings, emit, log, phase }) {
  ...
  // Cherche les URLs sources les plus proches des prix retenus par le LLM,
  // pour permettre à l'utilisateur de cliquer et vérifier dans l'overlay.
  function findClosestSource(targetPrice, kinds) {
    if (!targetPrice) return null;
    const candidates = [];
    for (const k of kinds) for (const p of (priceSummary.by_kind?.[k] || [])) candidates.push(p);
    if (!candidates.length) return null;
    let best = null;
    let bestDelta = Infinity;
    for (const c of candidates) {
      const delta = Math.abs(c.amount_eur - targetPrice);
      if (delta < bestDelta) { bestDelta = delta; best = c; }
    }
    return best ? { url: best.source, name: best.source_name, amount: best.amount_eur } : null;
  }
  const msrpSource = findClosestSource(synth.msrp_eur, ["msrp"]);
  const retailSource = findClosestSource(synth.retail_eur, ["retail", "current", "sale"]);
