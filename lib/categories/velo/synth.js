...
export function computeMarketFromNew(msrp, retail, year) {
  ...
  // occasion en retail. On le ignore pour ne pas pourrir le calcul.
  let newPrice;
...
// Surcharge : si on a beaucoup de comparables (>= 5) MEME sans tier-match, on privilégie
// la médiane globale car c'est un signal terrain robuste, plus fiable que le calcul
// théorique msrp * décote (qui peut être ruiné par un MSRP mal estimé).
export function computeBikeDealScores({ asking, msrp, retail, year, lbcMedianTier, lbcMedianGlobal, comparablesCount = 0 }) {
  // Si on a >=5 comparables globaux et pas de tier-match, on les utilise comme
  // signal principal (pas juste fallback).
  let usedTier = lbcMedianTier;
  let usedGlobal = lbcMedianGlobal;
  if (!lbcMedianTier && lbcMedianGlobal && comparablesCount >= 5) {
    // On promeut le global au rang de "tier" pour qu'il soit utilisé comme signal fiable.
