...
DECOTE VTT/velo MUSCULAIRE occasion (% du prix neuf catalogue):
- < 4 ans : 50-70%
- 4-7 ans : 25-40%
- 8-12 ans : 12-22%  (obsolescence techno)
- > 12 ans : 5-15%

DECOTE VAE / VTTAE occasion : MOINS FORTE (moteur reste valorise, marche tendu)
- < 3 ans : 60-80%
- 3-5 ans : 50-60%
- 5-8 ans : 35-50%
- 8-12 ans : 20-30%
- > 12 ans : 10-20%
EXEMPLE : Orbea Wild 2020 (5 ans, VAE haut de gamme) : prix occasion typique
~50% du MSRP. Si MSRP=8500, marche occasion ≈ 4000-4500 EUR.
...
4. msrp_eur : prix CATALOGUE constructeur (RRP), provient des signaux 'msrp' web ou
   ta connaissance du modele. JAMAIS depuis les comparables LBC/Troc Velo (ce sont
   des prix occasion, pas catalogue). null si pas de signal fiable.
5. retail_eur : prix NEUF en boutique chez gros revendeur (Alltricks/Bike-Discount/
   Probikeshop/Bike24/Starbike). Provient des signaux 'retail' web UNIQUEMENT.
   JAMAIS depuis les comparables LBC/Troc Velo. JAMAIS depuis Upway/Buycycle (qui
   vendent du reconditionne, pas du neuf). null si pas de signal 'retail' web.
   retail_source : nom du revendeur (string), ou null si retail_eur=null.
   INTERDIT : retail_source = "LBC" / "Leboncoin" / "Troc Velo" / "Comparables".
...
11. cons : 2-4 bullets concis. INTERDIT :
   - speculation arnaque, ville/date
   - mention de l'absence de signal web / de comparables / de donnees marche
     (l'utilisateur n'a rien a faire de ces meta-informations sur ta confiance ;
     ce sont des points UTILES sur le velo lui-meme qu'il faut lister, pas des
     limites de ton analyse)
...
// Décote VTT/vélo musculaire (cf bike-ia-agent DECOTE_RULES_BIKE)
function decoteFactorMusculaire(age) {
  if (age < 4) return 0.6;
...
// Décote VAE / VTTAE : moins forte. Le moteur reste valorisé, la batterie reste
// utilisable, le marché VAE d'occasion est tendu (forte demande). Cf observations
// LBC : un Orbea Wild 2020 se vend ~50% du prix neuf 5 ans plus tard.
function decoteFactorVae(age) {
...
export function decoteFactor(year, opts = {}) {
  ...
  return opts.electric ? decoteFactorVae(age) : decoteFactorMusculaire(age);
}

export function computeMarketFromNew(msrp, retail, year, opts = {}) {
  // Sanity check : si retail < msrp * 0.7, le LLM a probablement classé un prix
  // occasion en retail (les revendeurs neufs decotent rarement plus de 30%).
  // Dans ce cas on prend msrp comme reference.
  let newPrice;
  if (msrp && retail && retail < msrp * 0.7) {
    newPrice = msrp;
  } else {
...
// Surcharge : si on a >=2 comparables (memes sans tier-match), on privilégie la
// médiane terrain plutôt que le calcul théorique msrp*décote (qui peut être ruiné
// par un MSRP halluciné ou une décote inadaptée).
export function computeBikeDealScores({
  asking, msrp, retail, year, electric,
  lbcMedianTier, lbcMedianGlobal, comparablesCount = 0,
}) {
  let usedTier = lbcMedianTier;
  let usedGlobal = lbcMedianGlobal;
  if (!lbcMedianTier && lbcMedianGlobal && comparablesCount >= 2) {
    usedTier = lbcMedianGlobal;
  }
  return combineDealScores({
    asking,
    marketFromNew: computeMarketFromNew(msrp, retail, year, { electric }),
    marketFromUsedTier: usedTier,
    marketFromUsedGlobal: usedGlobal,
    comparablesCount,
  });
}
