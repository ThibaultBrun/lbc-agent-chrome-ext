...
export async function fetchLbcComparables(identity, currentUrl, log = () => {}) {
  ...
  let html;
  // Stratégie 1 : delegate au content script (cookies + Datadome OK car même origine)
  try {
    ...
    // Stratégie 2 : fetch direct depuis le service worker (peut renvoyer 403 Datadome)
    try {
      const r = await httpGetWithCookies(url, { timeout: 15000 });
      html = r.text;
