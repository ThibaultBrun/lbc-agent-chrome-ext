// Port de bike_agent/identity.py : extraction d'identité + détection électrique/tier.
...
// ─── Schéma JSON de l'extracteur (port de benchmark_extraction.schema) ──
...
// Post-traitement déterministe (regex + heuristiques). Comble ce que le LLM aurait raté.
export function postProcessIdentity(data, adText, attributes = {}) {
  const out = { ...data };
  const text = String(adText || "");
  const lower = text.toLowerCase();

  // Année : si LLM a mis null mais qu'il y a un 20XX dans le texte
  if (!out.annee) {
    const m = text.match(/\b(20[0-3]\d)\b/);
    if (m) out.annee = parseInt(m[1], 10);
  }

  // Taille de roues : depuis attributs LBC ou texte
  if (!out.taille_roues) {
  ...
  // Taille cadre : attributs ou regex
  if (!out.taille_cadre) {
  ...
  // Tier (S-Works/H10/etc.) si LLM ne l'a pas trouvé
  if (!out.version) {
    const tier = detectVariantTier(text);
    if (tier) out.version = tier;
  }

  // Détection électrique (très fiable côté heuristique)
