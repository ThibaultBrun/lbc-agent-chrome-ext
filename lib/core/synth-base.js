...
// Mappe un ratio prix_demandé / prix_marché vers un score 0-100.
// Calibrage observe : un asking 5-15% au-dessus de la mediane n'est pas "tres
// cher", c'est juste "un peu au-dessus". Idem un asking 30% au-dessus n'est pas
// catastrophique, c'est cher mais negociable.
export function ratioToScore(r) {
  if (r >= 1.50) return 5;     // tres cher (>50% au-dessus)
  if (r >= 1.30) return 20;    // cher (30-50% au-dessus)
  if (r >= 1.15) return 35;    // au-dessus marche
  if (r >= 1.05) return 45;    // legerement au-dessus
  if (r >= 0.95) return 55;    // au prix du marche
  if (r >= 0.85) return 70;    // sous marche -5/-15%
  if (r >= 0.70) return 82;    // bonne affaire -15/-30%
  if (r >= 0.55) return 90;    // tres bonne affaire -30/-45%
  return 95;                   // exceptionnel <-45%
}
