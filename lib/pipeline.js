...
// Adaptateur LLM. Côté service worker on ne peut appeler que Ollama directement.
// Les appels Gemini Nano passent par un canal vers le content script.
export class LlmRouter {
  constructor({ backend, nanoBridge }) {
    this.backend = backend;          // { kind: 'ollama'|'nano'|'none', url? }
    this.nanoBridge = nanoBridge;    // async ({prompt, format, system, stream}) => string
  }
  ...
  async json({ system, prompt, schema, model, temperature = 0, timeout = 60000 }) {
    ...
    if (this.backend.kind === "nano" && this.nanoBridge) {
      const out = await this.nanoBridge({ prompt, system, schema });
      return safeParseJson(out);
    }
  ...
  async text({ system, prompt, model, temperature = 0.2, timeout = 90000, onChunk }) {
    ...
    if (this.backend.kind === "nano" && this.nanoBridge) {
      return this.nanoBridge({ prompt, system, stream: !!onChunk, onChunk });
    }
    throw new Error("No LLM backend available");
  }
}
