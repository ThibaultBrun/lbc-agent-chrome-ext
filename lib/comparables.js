// Comparables d'occasion : Troc Vélo + Leboncoin.
// Stratégie : depuis le service worker, fetch avec credentials=include — les cookies
// utilisateurs (Datadome/CF) sont envoyés, les sites répondent comme à l'utilisateur.
// On parse le HTML avec regex (DOMParser n'est pas dispo dans un service worker MV3).

import { httpGetWithCookies, normalizeSpace } from "./utils.js";
import { compactIdentity, detectElectric, wheelSizeInches } from "./identity.js";

// ─── Troc Vélo ────────────────────────────────────────────────────────────

function buildTrocVeloUrl(query) {
  // Format public : https://www.troc-velo.com/recherche?q=<query>
  const q = encodeURIComponent(query);
  return `https://www.troc-velo.com/recherche?q=${q}`;
}

function parseTrocVeloResults(html) {
  // Les annonces TV sont dans des cartes avec data-attributes ; on cible large.
  const out = [];
  // Pattern tolérant : <article>...<a href="/petites-annonces/...">titre</a> ...prix... </article>
...
export async function fetchTrocVeloComparables(identity, log = () => {}) {
  ...
  log(`[trocvelo:search] ${query}`);

  let html;
  try {
    const r = await httpGetWithCookies(buildTrocVeloUrl(query), { timeout: 15000 });
    html = r.text;
  } catch (e) {
...
function buildLbcUrl(query) {
  // Catégorie LOISIRS_VELOS = 24
  const q = encodeURIComponent(query);
  return `https://www.leboncoin.fr/recherche?category=24&text=${q}&sort=time`;
}

function parseLbcResults(html) {
  // LBC charge ses annonces via Next.js — on cherche __NEXT_DATA__ JSON puis fallback regex.
  const out = [];
  ...
  // Fallback : regex simple sur les cartes (titre + prix)
...
export async function fetchLbcComparables(identity, currentUrl, log = () => {}) {
  ...
  if (identity.annee) parts.push(String(identity.annee));
  const query = parts.join(" ");
  log(`[lbc:search] ${query}`);

  let html;
  try {
    const r = await httpGetWithCookies(buildLbcUrl(query), { timeout: 15000 });
    html = r.text;
  } catch (e) {
...
export async function fetchAllComparables(identity, currentUrl, settings, log = () => {}) {
  const tasks = [];
  if (settings.enableTrocVeloComparables !== false) tasks.push(fetchTrocVeloComparables(identity, log).catch(() => []));
  if (settings.enableLbcComparables !== false) tasks.push(fetchLbcComparables(identity, currentUrl, log).catch(() => []));
